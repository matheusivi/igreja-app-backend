...
const MockPedidoOracaoRepository = PedidoOracaoRepository as jest.MockedClass<
  typeof PedidoOracaoRepository
>;
...
describe("PedidoOracaoService", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    pedidoRepo =
      new MockPedidoOracaoRepository() as jest.Mocked<PedidoOracaoRepository>;
    ...
    pedidoRepo.contar = jest.fn().mockResolvedValue(2);
  });
  ...
  describe("list", () => {
    it("deve listar pedidos de oração sem filtros", async () => {
      const pedidos = [
        makePedido(),
        makePedido({ id: 2, descricaoPedido: "Outro pedido" }),
      ];
      pedidoRepo.listar.mockResolvedValue(pedidos);
      pedidoRepo.contar.mockResolvedValue(2);
    ...
    it("deve aplicar filtro de busca corretamente", async () => {
      ...
      expect(pedidoRepo.listar).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            descricaoPedido: { contains: "família", mode: "insensitive" },
          },
          take: 10,
          skip: 10,
        }),
    ...
    it("deve retornar lista vazia quando não há pedidos", async () => {
      ...
      pedidoRepo.contar.mockResolvedValue(0);
  ...
  describe("delete", () => {
    it("deve permitir que o autor delete seu próprio pedido", async () => {
      pedidoRepo.buscarParaPermissao.mockResolvedValue({
        id: 1,
        autorUsuarioId: 1,
      });
    ...
    it("deve lançar erro 403 se Membro tentar deletar pedido de outro usuário", async () => {
      ...
      await expect(service.delete(1, 99, Perfis.MEMBRO)).rejects.toThrow(
        new AppError("Você não tem permissão para excluir este pedido", 403),
      );
    ...
    it("deve lançar erro 403 se Líder tentar deletar pedido de outro usuário", async () => {
      ...
      await expect(service.delete(1, 99, Perfis.LIDER)).rejects.toThrow(
        new AppError("Você não tem permissão para excluir este pedido", 403),
      );
