...
describe("ConteudoService", () => {
  ...
  describe("create", () => {
    it("deve criar um conteúdo com sucesso quando usuário existe", async () => {
      ...
      expect(mockConteudoRepository.criar).toHaveBeenCalledWith(
  ...
  describe("list", () => {
    it("deve listar conteúdos com filtros padrão", async () => {
      ...
      mockConteudoRepository.contar.mockResolvedValue(1); 
  ...
  describe("delete", () => {
    it("deve deletar quando o usuário é o autor", async () => {
      mockConteudoRepository.buscarParaPermissao.mockResolvedValue(
        mockConteudo as any,
      );
    ...
    it("deve lançar erro de permissão", async () => {
      ...
      await expect(service.delete(10, 999, "Usuario")).rejects.toThrow(
