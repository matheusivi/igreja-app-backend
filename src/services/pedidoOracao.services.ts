...
export class PedidoOracaoService {
  ...
  public async update(
    ...
    const pedidoExistente =
      await this.pedidoOracaoRepository.buscarParaPermissao(pedidoId);

    if (!pedidoExistente) {
      throw new AppError("Pedido de oração não encontrado.", 404);
    }

    if (pedidoExistente.autorUsuarioId !== usuarioId) {
      throw new AppError(
    ...
    const pedidoAtualizado = await this.pedidoOracaoRepository.atualizar(
      pedidoId,
      descricaoPedido,
    );

    return this.formatarResponse(pedidoAtualizado);
  }
  ...
  public async delete(
    ...
    if (!podeDeletar) {
      throw new AppError(
        "Você não tem permissão para excluir este pedido",
        403,
      );
    }
