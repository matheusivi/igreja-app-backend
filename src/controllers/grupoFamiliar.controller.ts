...
import { CreateGrupoFamiliarSchema, ConvidarMembroSchema, ResponderConviteSchema, } from '../validation/grupoFamiliar.validation';
...
export class GrupoFamiliarController {
    ...
    public create = async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        const grupo = await this.grupoFamiliarService.create(validatedData, usuarioId);
    ...
    public responderConvite = async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        await this.grupoFamiliarService.responderConvite(membroId, validatedData, usuarioId);

        res.status(200).json({
            success: true,
            message: validatedData.status === 'aceito' ? 'Convite aceito' : 'Convite recusado',
        });
    };
    ...
    public getByUsuario = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const resultado = await this.grupoFamiliarService.getByUsuario(usuarioId, page, limit);
  ...
  res.status(200).json({
    ...
    public removerMembro = async (req: AuthRequest, res: Response): Promise<void> => {
        const grupoId = Number(req.params.grupoId);
        ...
        await this.grupoFamiliarService.removerMembro(grupoId, membroUsuarioId, usuarioId, perfil);
